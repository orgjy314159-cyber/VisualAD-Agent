...
> 我做了一个面向工业缺陷检测的视觉 Agent Demo。系统不是简单调用一个视觉模型，而是把图像质量检测、异常区域检测、可视化、多模态解释和报告生成封装成多个工具，由 Agent 按照质检流程进行调度。用户上传图片后，Agent 自动完成从质量评估到报告输出的全链路。

### 版本 2（详细，3-5 分钟项目介绍）

> 我做了一个面向工业缺陷检测的视觉 Agent Demo。系统不是简单调用一个视觉模型，而是把图像质量检测、异常区域检测、可视化、多模态解释和报告生成封装成多个工具，由 Agent 按照质检流程进行调度。
>
> 用户上传图片后，Agent 首先判断图像是否模糊、过暗或过曝；然后调用异常检测工具定位疑似缺陷区域；接着生成可视化标注图；最后结合检测结果生成结构化 JSON 和自然语言质检报告。
>
> 这个项目的重点是把视觉模型能力工程化，形成一个可交互、可解释、可扩展的 AI 应用流程。后续可以接入 YOLO、SAM、VLM 或我之前的 3D 异常检测模型 SPA-SDF，从而扩展到更复杂的工业检测场景。

### 能力映射

| 展示能力 | 对应模块 |
|---------|---------|
| 计算机视觉与异常检测基础 | `image_quality_tool`、`anomaly_detection_tool` |
| 模型能力封装为可复用工具 | `tools/` 模块化设计，统一输入输出 |
| Agent 工具调用与流程编排 | `agent.py` 工作流调度 |
| 需求到实现的工程闭环 | Phase 0→11 完整开发链路 |
| 科研经验迁移到工程 | 异常检测算法 → 可部署 AI 应用 |

## 简历描述版本

```text
VisualAD-Agent：面向工业缺陷检测的多模态视觉质检 Agent

- 设计并实现面向工业图像质检场景的视觉 Agent 系统，支持图像上传、质量评估、
  异常区域检测、可视化标注与结构化报告生成。
- 将图像质量检测、异常检测、可视化、多模态解释和报告生成封装为独立 tools，
  由 Agent 按照可控工作流自动调度，实现端到端质检流程。
- 基于 OpenCV 构建轻量异常检测 baseline（背景残差 + 自适应阈值 + 形态学处理），
  并设计统一 Pydantic 数据结构，输出缺陷位置、异常分数、严重程度和人工复核建议。
- 搭建 Streamlit 交互页面，支持检测结果可视化、JSON 展示和 Markdown 报告下载，
  为后续接入 YOLO、SAM、VLM 和 3D 异常检测模型预留扩展接口。
```
